...
void Misc::watermark() noexcept
{
    ...
    ImGui::Text("daniel.krupinski | %s | %s", fps.c_str(), ping.c_str());
...
void Misc::killSay(GameEvent& event) noexcept
{
    ...
    srand(time(NULL));
    auto randomMessage = rand() % 7;

    std::string killMessage = "";
    switch (randomMessage) {
    case 0:
        killMessage = "getting tapped by " + cheatNames[rand() % ARRAYSIZE(cheatNames)] + " can't be me.";
        break;
    case 1:
        killMessage = "IQ? Pwned by " + cheatNames[rand() % ARRAYSIZE(cheatNames)];
        break;
    case 2:
        killMessage = "get good. get " + cheatNames[rand() % ARRAYSIZE(cheatNames)];
        break;
    case 3:
        killMessage = "Osiris - best free cheat made by the one and only Daniel Krupinski.";
        break;
    case 4:
        killMessage = "Imagine pasting from Osiris LOL, can only be moonlight.uno devs ";
        break;
    case 5:
        killMessage = "Visit https://github.com/demitriusdemarcus and maybe follow if u like to get more resources for CSGO cheats.";
        break;
    case 6:
        killMessage = "Huge respect to https://github.com/notgoodusename for the new Osiris menu <3 .";
        break;
    }
...
void Misc::teamDamageCounter(GameEvent* event) noexcept {
    ...
    if (interfaces->engine->getPlayerForUserID(event->getInt("attacker")) == localPlayer) { // we attacked.
        ...
        if (ent && !ent->isOtherEnemy(localPlayer.get())) 
        { // teammate got hurt
